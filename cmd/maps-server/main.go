...
import (
	...
	maps "github.com/localitas/localitas-app-maps"
...
func serveAction(ctx context.Context, cmd *cli.Command) error {
	...
	a := maps.New(c, basePath)

	dbID, err := a.Install(ctx)
	if err != nil {
		return fmt.Errorf("install: %w", err)
	}
	log.Printf("Maps database ready: %s", dbID)
	...
	mux.HandleFunc("GET /health.json", maps.HandleHealth)
	...
	fmt.Printf("maps-server listening on http://localhost:%d\n", addr.Port)

	selfURL := fmt.Sprintf("http://localhost:%d", addr.Port)
	if err := c.RegisterService(ctx, "maps", selfURL); err != nil {
...
func migrateCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			log.Printf("Maps migrations complete (database: %s)", dbID)
			return nil
		},
	}
}
