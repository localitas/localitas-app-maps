...
```bash
localitas-core app-store add --name maps --compose ./docker-compose.yml --port 9207
localitas-core app-store start maps
```

Or open the App Store UI (package icon, top-right nav) and paste the `docker-compose.yml`.

The image is published to `ghcr.io/localitas/localitas-app-maps:latest`. To publish a new version:
